    ...
    <div class="d-sm-flex">
        ...
        <div class="page-side-nav c-scrollbar-light px-3 py-2">
            <ul class="nav nav-tabs flex-sm-column border-0" role="tablist" aria-orientation="vertical">
                ...
                <!-- Specifications -->
                <li class="nav-item">
                    <a class="nav-link" id="specifications-tab" href="#specifications_tab"
                        data-toggle="tab" data-target="#specifications_tab" type="button" role="tab" aria-controls="specifications_tab" aria-selected="false">
                        ...
                        <div class="bg-white p-3 p-sm-2rem">
                            ...
                            <div class="w-100">
                                <div class="row">
                                    <div class="col-xxl-7 col-xl-6">
                                        ...
                                        <div class="form-group row">
                                           ...
                                           <div class="col-xxl-9">
                                               <input type="text" class="form-control" name="model_number" placeholder="e.g 23323232" value="{{$product->model_number}}">
                                           </div>
                                        </div>
                                        <!-- Weight -->
                                        <div class="form-group row">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="">
                                        <textarea class="" id="summernote" name="description">{{ $product->getTranslation('description', $lang) }}</textarea>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="bg-white p-3 p-sm-2rem">
                            ...
                            <div class="form-group row d-none">
                                ...
                                <div class="col-md-9">
                                    ...
                                    <option value="local" <?php if ($product->video_provider == 'local') echo "selected"; ?> >{{translate('Local')}}</option>
                            ...
                            <!-- Video Link -->
                            <div class="form-group row d-none">
                                <label class="col-md-3 col-from-label">{{translate('Useful Link')}}</label>
                                <div class="col-md-9">
                            ...
                            <!-- Local Video Upload -->
                             <x-video-upload :videoThumbnailId="$product->video_thumbnail_id" :localVideoId="$product->local_video_id" />
                             <!-- PDF Specification -->
                               <div class="form-group row">
    <label class="col-md-3 col-form-label" for="signinSrEmail">{{translate('PDF Specifications')}}</label>
    <div class="col-md-9">
        <div class="input-group" data-toggle="aizuploader" data-type="document" data-multiple="true">
            ...
            <!-- Decode the JSON and pass the IDs to the input value -->
            <input type="hidden" name="pdf_ids" class="selected-files" value="{{ $product->pdf ? collect(json_decode($product->pdf, true))->pluck('id')->implode(',') : '' }}">
        </div>
        <div class="file-preview box sm"></div>
        
        <div id="pdf-titles-container" class="mt-3"></div>
    </div>
</div>

<!-- IES File Input -->
<div class="form-group row">
    <label class="col-md-3 col-form-label" for="signinSrEmail">{{translate('IES File')}}</label>
    <div class="col-md-9">
        <!-- This input will be for a single file -->
        <div class="input-group" data-toggle="aizuploader" data-type="document">
            <div class="input-group-prepend">
                <div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse')}}</div>
            </div>
            <div class="form-control file-amount">{{ translate('Choose File') }}</div>
            <!-- The selected IES file ID will be stored here -->
            <input type="hidden" name="ies_id" value="{{ $product->ies_id }}" class="selected-files">
        </div>
        <div class="file-preview box sm"></div>
    </div>
</div>
                              <!-- Video Links  -->
                            <div class="form-group row">
    <label class="col-md-3 col-form-label" for="videoLinks">{{translate('Useful Links')}}</label>
    <div class="col-md-9" id="videoLinksContainer">
        @php
            $video_links = json_decode($product->video_links, true) ?? [];
            $i = 0; // Initialize a counter for the array index
        @endphp

        @if(count($video_links) > 0)
            @foreach($video_links as $item)
                <div class="input-group mb-2">
                    <input type="text" class="form-control" name="video_links[{{ $i }}][title]" value="{{ $item['title'] ?? '' }}" placeholder="Enter video title">
                    <input type="url" class="form-control" name="video_links[{{ $i }}][link]" value="{{ $item['link'] ?? '' }}" placeholder="Enter video link">
                    <button class="btn btn-danger remove-link" type="button">{{translate('Remove')}}</button>
                </div>
                @php $i++; @endphp
            @endforeach
        @else
            {{-- Display one empty input pair if there are no existing links --}}
            <div class="input-group mb-2">
                <input type="text" class="form-control" name="video_links[0][title]" placeholder="Enter video title">
                <input type="url" class="form-control" name="video_links[0][link]" placeholder="Enter video link">
                <button class="btn btn-danger remove-link" type="button" style="display:none;">{{translate('Remove')}}</button>
            </div>
            @php $i++; @endphp
        @endif
    </div>
    <div class="col-md-9 offset-md-3 mt-2">
        <button type="button" class="btn btn-primary" id="addLinkBtn">{{translate('Add More Link')}}</button>
    </div>
</div>
                    ...
                    <div class="tab-pane fade" id="price_and_stocks" role="tabpanel" aria-labelledby="price-and-stocks-tab">
                     ...
                     <!-- Specifications -->
                    <div class="tab-pane fade" id="specifications_tab" role="tabpanel" aria-labelledby="specifications-tab">
                        
<div class="form-group row mb-2 mt-4">
   
    <div class="col-md-9" id="specificationsContainer">
        
        <div class="form-group row mb-2 mt-4">
            <div class="col-md-6">
                <label class="col-form-label">{{ translate('Property Name') }}</label>
            </div>
            <div class="col-md-6">
                <label class="col-form-label">{{ translate('Property Value') }}</label>
            </div>
        </div>
        @php
        ...
        @if(count($specifications) > 0)
            @foreach($specifications as $spec)
                <div class="input-group mb-2">
                    <input type="text" class="form-control" name="specifications[{{ $i }}][key]" value="{{ $spec['key'] ?? '' }}" placeholder="{{ translate('Key (e.g., Color)') }}">
                    <input type="text" class="form-control" name="specifications[{{ $i }}][value]" value="{{ $spec['value'] ?? '' }}" placeholder="{{ translate('Value (e.g., Red)') }}">
                    <button class="btn btn-danger remove-spec" type="button">{{ translate('Remove') }}</button>
                </div>
                @php $i++; @endphp
            @endforeach
        @else
            <!-- First input pair -->
            <div class="input-group mb-2">
                <input type="text" class="form-control" name="specifications[0][key]" placeholder="{{ translate('Key (e.g., Color)') }}">
                <input type="text" class="form-control" name="specifications[0][value]" placeholder="{{ translate('Value (e.g., Red)') }}">
                <button class="btn btn-danger remove-spec" type="button" style="display:none;">{{ translate('Remove') }}</button>
            </div>
            @php $i++; @endphp
        @endif
    </div>
    <div class="col-md-9 offset-md-3 mt-2">
        <button type="button" class="btn btn-primary" id="addSpecBtn">{{ translate('Add More Specification') }}</button>
    </div>
</div>
...
<script>
    // Initialize the link index with the number of existing links.
    // This ensures that new links added will have the correct array index.
    let linkIndex = {{ count($video_links) }};

    document.getElementById('addLinkBtn').addEventListener('click', function() {
        let container = document.getElementById('videoLinksContainer');
        let newInput = document.createElement('div');
        newInput.classList.add('input-group', 'mb-2');
        newInput.innerHTML = `
            <input type="text" class="form-control" name="video_links[${linkIndex}][title]" placeholder="Enter video title">
            <input type="url" class="form-control" name="video_links[${linkIndex}][link]" placeholder="Enter video link">
            <button class="btn btn-danger remove-link" type="button">{{translate('Remove')}}</button>
        `;
    ...
    document.getElementById('videoLinksContainer').addEventListener('click', function(e) {
        if (e.target.classList.contains('remove-link')) {
            e.target.closest('.input-group').remove();
        }
    });

    // Handle the visibility of the "Remove" button for the first input,
    // in case it's a new, empty field.
    const firstRemoveBtn = document.querySelector('.input-group .remove-link');
    const firstLinkInput = document.querySelector('.input-group input[name="video_links[0][link]"]');

    if (firstLinkInput && firstRemoveBtn) {
        firstLinkInput.addEventListener('input', function() {
            if (this.value.trim() !== '' || document.querySelector('input[name="video_links[0][title]"]').value.trim() !== '') {
...
<script>
    ...
    document.getElementById('addSpecBtn').addEventListener('click', function() {
        let container = document.getElementById('specificationsContainer');
        let newInput = document.createElement('div');
        newInput.classList.add('input-group', 'mb-2');
        newInput.innerHTML = `
            <input type="text" class="form-control" name="specifications[${specIndex}][key]" placeholder="{{ translate('Key (e.g., Color)') }}">
            <input type="text" class="form-control" name="specifications[${specIndex}][value]" placeholder="{{ translate('Value (e.g., Red)') }}">
            <button class="btn btn-danger remove-spec" type="button">{{ translate('Remove') }}</button>
        `;
        container.appendChild(newInput);
        specIndex++;
    ...
    // Handle the visibility of the "Remove" button for the first input pair.
    const removeBtn = document.querySelector('.input-group .remove-spec');
    const firstKeyInput = document.querySelector('.input-group input[name="specifications[0][key]"]');
    const firstValueInput = document.querySelector('.input-group input[name="specifications[0][value]"]');

    function checkFirstSpecInput() {
        if (firstKeyInput.value.trim() !== '' || firstValueInput.value.trim() !== '') {
    ...
    if (firstKeyInput && firstValueInput) {
        firstKeyInput.addEventListener('input', checkFirstSpecInput);
        firstValueInput.addEventListener('input', checkFirstSpecInput);
    }
</script>


<script>
    // Get the hidden input and the titles container
    const selectedFilesInput = document.querySelector('input[name="pdf_ids"]');
    const titlesContainer = document.getElementById('pdf-titles-container');
    let lastKnownValue = selectedFilesInput.value;
    let savedTitlesInitialized = false;

    // A JavaScript object to hold the pre-saved titles
    let savedTitles = {};

    // A function to initialize the savedTitles object only once
    const initializeSavedTitles = () => {
        if (!savedTitlesInitialized) {
            @if ($product->pdf)
                try {
                    const savedData = JSON.parse('{!! addslashes($product->pdf) !!}');
                    savedData.forEach(item => {
                        savedTitles[item.id] = item.title;
                    });
                    savedTitlesInitialized = true;
                } catch (e) {
                    console.error("Failed to parse PDF JSON data:", e);
                }
            @endif
        }
    };

    // Function to generate the title inputs based on selected IDs
    const updateTitleInputs = () => {
        // Ensure saved titles are initialized before proceeding
        initializeSavedTitles();
        
        const currentIds = selectedFilesInput.value;
        // Only run the update if the value has actually changed
        // or if it's the initial run to populate the fields
        if (currentIds !== lastKnownValue || !titlesContainer.hasChildNodes()) {
            lastKnownValue = currentIds;
            
            // Clear any existing title inputs
            titlesContainer.innerHTML = '';
            
            // Get the selected PDF file IDs from the hidden input's value
            const pdfIds = currentIds.split(',').filter(id => id.length > 0);
            
            if (pdfIds.length > 0) {
                // Loop through each selected PDF and create a title input
                pdfIds.forEach(pdfId => {
                    const titleInputGroup = document.createElement('div');
                    titleInputGroup.className = 'form-group d-flex align-items-center';
                    
                    const input = document.createElement('input');
                    input.type = 'text';
                    input.name = `pdf_titles[${pdfId}]`;
                    input.className = 'form-control mr-2';
                    input.placeholder = 'Enter title for PDF ID: ' + pdfId;
                    input.required = true;
                    // Pre-fill the input with the saved title if it exists
                    if (savedTitles[pdfId]) {
                        input.value = savedTitles[pdfId];
                    }
                    
                    const pdfIdSpan = document.createElement('span');
                    pdfIdSpan.className = 'text-muted';
                    pdfIdSpan.textContent = `(ID: ${pdfId})`;
                    
                    titleInputGroup.appendChild(input);
                    titleInputGroup.appendChild(pdfIdSpan);
                    titlesContainer.appendChild(titleInputGroup);
                });
            }
        }
    };
    ...
    setInterval(updateTitleInputs, 200);
</script>
