...
@php 
   ...
   $enableAskForPrice = get_setting('enable_product_ask_for_price') == 1;
    ...
    @if ($detailedProduct->brand != null)
        <div class="d-flex flex-wrap align-items-center mb-3">
            ...
            <a href="{{ route('products.brand', $detailedProduct->brand->slug) }}"
                class="text-reset hov-text-primary text-primary fs-14 fw-700" style="text-decoration:underline;">{{ $detailedProduct->brand->name }}</a>
        </div>
    @endif
    @if ($detailedProduct->model_number != null)
        <div class="d-flex flex-wrap align-items-center mb-3">
            <span class="text-secondary fs-14 fw-400 mr-4 w-80px">{{ translate('Model') }}</span><br>
            <p class="text-reset fs-14 fw-700 mb-0 mt-0">{{ $detailedProduct->model_number }}</p>
        </div>
    @endif

    @if ($detailedProduct->main_category != null)
        <div class="d-flex flex-wrap align-items-center mb-3">
            <span class="text-secondary fs-14 fw-400 mr-4 w-80px">{{ translate('Category') }}</span><br>
            <a href="{{ route('products.category', $detailedProduct->main_category->slug) }}"
                class="text-reset hov-text-primary fs-14 fw-700 text-primary" style="text-decoration:underline;">{{ $detailedProduct->main_category->name }}</a>
        </div>
    @endif
    ...
    @else
        ...
        @else
            ...
            @if (home_price($detailedProduct) != home_discounted_price($detailedProduct))
                <div class="row no-gutters mb-3 {{ $enableAskForPrice == 1 ? 'd-none':'' }}">
                    <div class="col-sm-2">
    ...
    @if ($detailedProduct->auction_product != 1)
        <form id="option-choice-form">
            ...
            <div class="row no-gutters pb-3 d-none" id="chosen_price_div">
                <div class="col-sm-2 {{ $enableAskForPrice == 1 ? 'd-none':'' }}">
                    <div class="text-secondary fs-14 fw-400 mt-1">{{ translate('Total Price') }}</div>
                </div>
                <div class="col-sm-10 {{ $enableAskForPrice == 1 ? 'd-none':'' }}">
        ...
        <div class="mt-3">
            @if ($detailedProduct->digital == 0)
                ...
                @else
                   ...
                   @if($enableAskForPrice == 1)

                  <a href="https://wa.me/{{ env('WHATSAPP_NUMBER') }}?text={{ urlencode(url()->current()) }}" target="_blank"
                        class="btn btn-secondary-base mr-2 fw-600 min-w-150px rounded-0 text-white">
                     <i class="las la-shopping-baG"></i> {{ translate('Ask for Price') }}
                  </a>
