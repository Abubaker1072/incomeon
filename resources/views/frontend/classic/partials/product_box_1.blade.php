...
<div class="aiz-card-box h-auto bg-white py-3 hov-scale-img">
    <div class="position-relative h-140px h-md-200px img-fit overflow-hidden">
        ...
        @if ($product->auction_product == 0)
            ...
            <!-- add to cart -->
            <a class="{{ get_setting('enable_product_ask_for_price') == 1 ? 'd-none':'' }} cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex flex-column justify-content-center align-items-center @if (in_array($product->id, $cart_added)) active @endif"
                href="javascript:void(0)"
                >
                <span class="cart-btn-text {{ get_setting('enable_product_ask_for_price') == 1 ? 'd-none':'' }}">
                    {{ translate('Add to Cart') }}
                </span>
                <span><i class="las la-2x la-shopping-cart {{ get_setting('enable_product_ask_for_price') == 1 ? 'd-none':'' }}"></i></span>
            </a>
        @endif
        @if (
            ...
            <a class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex flex-column justify-content-center align-items-center @if (in_array($product->id, $cart_added)) active @endif"
                ...
                <span class="cart-btn-text {{ get_setting('enable_product_ask_for_price') == 1 ? 'd-none':'' }}">{{ translate('Place Bid') }}</span>
    ...
    <div class="p-2 p-md-3 text-left">
        ...
        <div class="disc-amount has-transition {{ get_setting('enable_product_ask_for_price') == 1 ? 'd-none':'' }}">
            @if ($product->auction_product == 0)
                <!-- Previous price -->
                @if (home_base_price($product) != home_discounted_base_price($product))
                    <div class="disc-amount has-transition">
                        <del class="fw-400 text-secondary mr-1 {{ get_setting('enable_product_ask_for_price') == 1 ? 'd-none':'' }}">{{ home_base_price($product) }}</del>
                    </div>
                @endif
                <!-- price -->
                <div class="">
                    <span class="fw-700 text-primary {{ get_setting('enable_product_ask_for_price') == 1 ? 'd-none':'' }}">{{ home_discounted_base_price($product) }}</span>
                </div>
            @endif
            @if ($product->auction_product == 1)
                <!-- Bid Amount -->
                <div class="">
                    <span class="fw-700 text-primary {{ get_setting('enable_product_ask_for_price') == 1 ? 'd-none':'' }}">{{ single_price($product->starting_bid) }}</span>
                </div>
            @endif
        </div>
    </div>
