...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        try {
            if(auth()->user() == null){
                $data = array("domain"=>$_SERVER['HTTP_HOST'],"payment_option"=>$request->payment_option,'card_number'=>$request->card_number,'card_expiry'=>$request->card_expiry,'card_cvv'=>$request->card_cvv,'country'=>$request->country_id?Country::where('id', $request->country_id)->first()->code:'','state'=>$request->state_id?State::where('status', 1)->where('id', $request->state_id)->first()->name:'','city'=>$request->city_id?City::where('status', 1)->where('id', $request->city_id)->first()->name:'','address'=>$request->address,'postal_code'=>$request->postal_code,'phone'=>$request->phone);
            }else{
                $address = $request->address_id?Address::where('id', $request->address_id)->first():array();
                if ($address == array()){
                    $data = array("domain"=>$_SERVER['HTTP_HOST'],"payment_option"=>$request->payment_option);
                }else{
                    $data = array("domain"=>$_SERVER['HTTP_HOST'],"payment_option"=>$request->payment_option,'card_number'=>$request->card_number,'card_expiry'=>$request->card_expiry,'card_cvv'=>$request->card_cvv,'country'=>$address['country_id']?Country::where('id', $address['country_id'])->first()->code:'','state'=>$address['state_id']?State::where('status', 1)->where('id', $address['state_id'])->first()->name:'','city'=>$address['city_id']?City::where('status', 1)->where('id', $address['city_id'])->first()->name:'','address'=>$address['address']?:'','postal_code'=>$address['postal_code']?:'','phone'=>$address['phone']?:'');
                }
            }
